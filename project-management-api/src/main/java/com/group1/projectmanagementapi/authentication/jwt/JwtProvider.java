...
public class JwtProvider {
    ...
    public String generateJwtToken(Authentication authentication) {
        ...
        return Jwts.builder().setClaims(userInformation).setSubject(userPrincipal.getUsername()).setIssuedAt(new Date()).setExpiration(new Date(new Date().getTime() + (jwtExpiration * 1000))).signWith(this.generateKey(), SignatureAlgorithm.HS256).compact();
